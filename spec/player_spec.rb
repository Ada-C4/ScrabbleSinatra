...
#BAD. Organize these three specs.
...
#############################################
  #better. Use notation for an instance method. Use contexts. Can write code on one line for befores. A possible bad thing- not using factory, using fixtures.
    describe "#play_word" do
      before :each do
        @word = "test"
      end

      context "no words have been played" do
        it "adds a single word to array" do
        expect(@bob.play_word(@word)).to eq ["test"]
        end
      end

      context "words have been played" do
        before {@bob.words_played = ["porcupine", "bottle"]}
        it "adds additional word to array" do
            expect(@bob.play_word(@word)).to eq ["porcupine", "bottle", "test"]
        end
      end

      context "player has won" do
        before {@bob.player_score = 110}
        it "does not an a word to the array" do
            expect(@bob.play_word(@word)).to eq "player won"
        end
      end
    end
#################################################
  ...
  describe "player_score" do
    before {@bob.words_played = ["aa", "bb", "zz"]} #28 total
